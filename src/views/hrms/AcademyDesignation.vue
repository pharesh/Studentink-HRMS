<template>
  <div class="page-content">
    ...
    <div class="page-header-row">
      <div>
        <div class="page-title-inline">Designation Master</div>
        <div class="page-sub">Manage designations — add, edit, or deactivate</div>
      </div>
      <button class="btn btn-primary" @click="openAdd">+ Add Designation</button>
    </div>
    ...
    <div class="toolbar">
      <input v-model="search" class="input-search" placeholder="Search designation name…" />
    </div>
      ...
      <table v-else class="data-table">
        ...
        <tbody>
          <tr v-if="filtered.length === 0">
            <td colspan="5" class="table-empty">No designations found.</td>
          </tr>
      ...
      <div v-if="modal.open" class="modal-backdrop" @click.self="modal.open = false">
        <div class="modal-box">
          <div class="modal-header">
            <span class="modal-title">{{ modal.id ? 'Edit Designation' : 'Add Designation' }}</span>
            <button class="modal-close" @click="modal.open = false">✕</button>
          </div>
          <div class="modal-body">
            <div class="form-group">
              <label class="form-label">Designation Name <span class="req">*</span></label>
              <input v-model="modal.Name" class="form-control" placeholder="e.g. Principal" maxlength="500" />
    ...
    <Teleport to="body">
      <div v-if="delModal.open" class="modal-backdrop" @click.self="delModal.open = false">
        <div class="modal-box modal-sm">
          <div class="modal-header">
            <span class="modal-title">Delete Designation</span>
...
async function load() {
  ...
  try {
    const { data } = await api.get('/users/hrms/designations', { params: { academy_id: academyId } })
    rows.value = data.data || []
  } catch { error.value = 'Failed to load designations.' }
...
function openEdit(row) { modal.value = { open: true, id: row.ADID, Name: row.Name }; formErr.value = {}; saveErr.value = '' }
...
async function save() {
  ...
  try {
    if (modal.value.id) {
      await api.put(`/users/hrms/designations/${modal.value.id}`, { Name: modal.value.Name.trim() })
    } else {
      await api.post('/users/hrms/designations', { AcademyID: parseInt(academyId), Name: modal.value.Name.trim() })
    }
...
async function toggleStatus(row) {
  try { await api.patch(`/users/hrms/designations/${row.ADID}/toggle-status`); await load() } catch {}
}

const delModal = ref({ open: false, id: null, name: '' })
const deleting = ref(false)
function confirmDelete(row) { delModal.value = { open: true, id: row.ADID, name: row.Name } }
async function doDelete() {
  deleting.value = true
  try { await api.delete(`/users/hrms/designations/${delModal.value.id}`); delModal.value.open = false; await load() }
