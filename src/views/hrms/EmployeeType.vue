<template>
  <div class="page-content">
    ...
    <div class="page-header-row">
      <div>
        <div class="page-title-inline">Employee Type Master</div>
        <div class="page-sub">Manage employee types — add, edit, or deactivate</div>
      </div>
      <button class="btn btn-primary" @click="openAdd">+ Add Employee Type</button>
    </div>
    ...
    <div class="toolbar">
      <input v-model="search" class="input-search" placeholder="Search employee type…" />
    </div>
      ...
      <table v-else class="data-table">
        ...
        <tbody>
          <tr v-if="filtered.length === 0">
            <td colspan="5" class="table-empty">No employee types found.</td>
          </tr>
          <tr v-for="(row, i) in filtered" :key="row.ETID">
      ...
      <div v-if="modal.open" class="modal-backdrop" @click.self="modal.open = false">
        <div class="modal-box">
          <div class="modal-header">
            <span class="modal-title">{{ modal.id ? 'Edit Employee Type' : 'Add Employee Type' }}</span>
            <button class="modal-close" @click="modal.open = false">✕</button>
          </div>
          <div class="modal-body">
            <div class="form-group">
              <label class="form-label">Name <span class="req">*</span></label>
              <input v-model="modal.Name" class="form-control" placeholder="e.g. Permanent, Temporary, Contractual" maxlength="200" />
              <span v-if="formErr.Name" class="field-err">{{ formErr.Name }}</span>
            </div>
    ...
    <Teleport to="body">
      <div v-if="delModal.open" class="modal-backdrop" @click.self="delModal.open = false">
        <div class="modal-box modal-sm">
          <div class="modal-header">
            <span class="modal-title">Delete Employee Type</span>
            <button class="modal-close" @click="delModal.open = false">✕</button>
          </div>
          <div class="modal-body">
            <p>Are you sure you want to delete <strong>{{ delModal.name }}</strong>?</p>
          </div>
          <div class="modal-footer">
...
async function load() {
  ...
  try {
    const { data } = await api.get('/users/hrms/employee-types', { params: { academy_id: academyId } })
    rows.value = data.data || []
  } catch { error.value = 'Failed to load employee types.' }
...
function openAdd()     { modal.value = { open: true, id: null, Name: '' }; formErr.value = {}; saveErr.value = '' }
function openEdit(row) { modal.value = { open: true, id: row.ETID, Name: row.Name }; formErr.value = {}; saveErr.value = '' }

async function save() {
  formErr.value = {}; saveErr.value = ''
  if (!modal.value.Name.trim()) { formErr.value.Name = 'Name is required'; return }
  saving.value = true
  try {
    if (modal.value.id) {
      await api.put(`/users/hrms/employee-types/${modal.value.id}`, { Name: modal.value.Name.trim() })
    } else {
      await api.post('/users/hrms/employee-types', { AcademyID: parseInt(academyId), Name: modal.value.Name.trim() })
    }
...
async function toggleStatus(row) {
  try { await api.patch(`/users/hrms/employee-types/${row.ETID}/toggle-status`); await load() } catch {}
}

const delModal = ref({ open: false, id: null, name: '' })
const deleting = ref(false)
function confirmDelete(row) { delModal.value = { open: true, id: row.ETID, name: row.Name } }
async function doDelete() {
  deleting.value = true
  try { await api.delete(`/users/hrms/employee-types/${delModal.value.id}`); delModal.value.open = false; await load() }
