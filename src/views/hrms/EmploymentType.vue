<template>
  <div class="page-content">
    ...
    <div class="page-header-row">
      <div>
        <div class="page-title-inline">Employment Type Master</div>
        <div class="page-sub">Manage employment types — add, edit, or deactivate</div>
      </div>
      <button class="btn btn-primary" @click="openAdd">+ Add Employment Type</button>
    </div>
    ...
    <div class="toolbar">
      <input v-model="search" class="input-search" placeholder="Search employment type…" />
    </div>
      ...
      <table v-else class="data-table">
        ...
        <tbody>
          <tr v-if="filtered.length === 0">
            <td colspan="5" class="table-empty">No employment types found.</td>
          </tr>
      ...
      <div v-if="modal.open" class="modal-backdrop" @click.self="modal.open = false">
        <div class="modal-box">
          <div class="modal-header">
            <span class="modal-title">{{ modal.id ? 'Edit Employment Type' : 'Add Employment Type' }}</span>
            <button class="modal-close" @click="modal.open = false">✕</button>
          </div>
          <div class="modal-body">
            <div class="form-group">
              <label class="form-label">Name <span class="req">*</span></label>
              <input v-model="modal.Name" class="form-control" placeholder="e.g. Full-Time, Part-Time, Contract" maxlength="200" />
    ...
    <Teleport to="body">
      <div v-if="delModal.open" class="modal-backdrop" @click.self="delModal.open = false">
        <div class="modal-box modal-sm">
          <div class="modal-header">
            <span class="modal-title">Delete Employment Type</span>
...
async function load() {
  ...
  try {
    const { data } = await api.get('/users/hrms/employment-types', { params: { academy_id: academyId } })
    rows.value = data.data || []
  } catch { error.value = 'Failed to load employment types.' }
...
function openEdit(row) { modal.value = { open: true, id: row.Id, Name: row.Name }; formErr.value = {}; saveErr.value = '' }
...
async function save() {
  ...
  try {
    if (modal.value.id) {
      await api.put(`/users/hrms/employment-types/${modal.value.id}`, { Name: modal.value.Name.trim() })
    } else {
      await api.post('/users/hrms/employment-types', { AcademyID: parseInt(academyId), Name: modal.value.Name.trim() })
    }
...
async function toggleStatus(row) {
  try { await api.patch(`/users/hrms/employment-types/${row.Id}/toggle-status`); await load() } catch {}
}

const delModal = ref({ open: false, id: null, name: '' })
const deleting = ref(false)
function confirmDelete(row) { delModal.value = { open: true, id: row.Id, name: row.Name } }
async function doDelete() {
  deleting.value = true
  try { await api.delete(`/users/hrms/employment-types/${delModal.value.id}`); delModal.value.open = false; await load() }
